...
vim.pack.add({
	{ src = "https://github.com/vague2k/vague.nvim", version = "v1.4.1" },
...
local function pack_clean()
	...
	if #unused_plugins == 0 then return end
...
vim.api.nvim_create_autocmd("ColorScheme", {
	group = color_group,
	callback = function()
		if vim.t.color then vim.cmd("colorscheme " .. vim.t.color)
		else vim.cmd("colorscheme " .. default_color) end
	end,
})
...
for i = 1, 8 do map({ "n", "t" }, "<Leader>" .. i, "<Cmd>tabnext " .. i .. "<CR>") end
map({ "n", "v", "x" }, ";", ":")
map({ "n", "v", "x" }, ":", ";")
...
local servers = { clangd = "c,c++", pyright = "python" }
